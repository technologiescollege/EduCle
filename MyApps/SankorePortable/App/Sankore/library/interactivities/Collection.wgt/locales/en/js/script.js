var sankoreLang = {
    ...
    edit: "Edit", 
    short_desc: "Enter your description here ...", 
    add: "Add new block",
    ...
    help: "Help",
    help_content: 
"<p> <h2>Collection</h2> </p>" +
"<p> <h3>Allow to present a collection of images</h3> </p>" +
...
function start(){
	...
	$("#wgt_show_images").text(sankoreLang.hide_images);
    ...
    if(window.sankore){
        if(sankore.preference("collection","")){
            var data = jQuery.parseJSON(sankore.preference("collection",""));
            importData(data);
        } else {
            showExample();
        }
    ...
    $("#wgt_show_images").click(function(){
        var tmp = $(this);
        if($(this).hasClass("open")){
            $(".imgs_cont").slideDown("1000", function(){
            ...
            $(".imgs_cont").slideUp("1000", function(){
				tmp.addClass("open");
				$("#wgt_show_images").text(sankoreLang.show_images);
            });
    ...
    $("#wgt_display, #wgt_edit").mouseover(function(){
        exportData();
    });
...
function exportData(){
	...
	if(array_to_export.length > 0)
	{
		array_to_export[0].show_images = show_images;
	}
	
    sankore.setPreference("collection", JSON.stringify(array_to_export));
}
    ...
    for(var i in data){
        if(data[i].tmp){
            ...
            if(data[i].mode == "edit"){          
				...
				$("#wgt_show_images").appendTo(container);
                    ...
                    drop: function(event, ui) {
                        if($(ui.draggable).parent().parent().html() == $(this).parent().html()){
                            if(this != $(ui.draggable).parent()[0]){
                                ...
                                tmp_cont.append("<img src='img/drop_img.png' style='margin-top: 11px;'/>");
                                var answers = "";
                                $(this).parent().find(".img_block").each(function(){
                                    answers += $(this).find("input").val();
                                });
                                if(tmp_cont.children()[0].tagName == "IMG")
                                    tmp_cont.removeClass("imgs_answers_green")
...
//                number = $("<div class='number_cont'>"+ (++tmp) +"</div>").appendTo(sub_container);
			...
			show_images = data[i].show_images;
			
			if(!show_images)
			{
				$(".imgs_cont").hide();
				
				$("#wgt_show_images").text(sankoreLang.show_images).addClass("open");
			}
        }
    }
}
...
//    var number = $("<div class='number_cont'>1</div>").appendTo(sub_container);
    ...
    $("<img src=\"img/drop_img.png\" style=\"display: inline;\" height=\"120\"/>").appendTo(img1);
    var img2 = $("<div class='img_block img_gray' style='text-align: center;'></div>");
    $("<img src=\"img/drop_img.png\" style=\"display: inline;\" height=\"120\"/>").appendTo(img2);
    var img3 = $("<div class='img_block img_gray' style='text-align: center;'></div>");
    $("<img src=\"img/drop_img.png\" style=\"display: inline;\" height=\"120\"/>").appendTo(img3);
    var img4 = $("<div class='img_block img_gray' style='text-align: center;'></div>");
    $("<img src=\"img/drop_img.png\" style=\"display: inline;\" height=\"120\"/>").appendTo(img4);
    var img5 = $("<div class='img_block img_gray' style='text-align: center;'></div>");
    $("<img src=\"img/drop_img.png\" style=\"display: inline;\" height=\"120\"/>").appendTo(img5);  
...
function changeStyle(val){
    switch(val){
        case "1":
            ...
            $("#wgt_show_images").removeClass("pad_color");
        ...
        case "2":
            ...
            $("#wgt_show_images").addClass("pad_color");
...
function onDropTarget(obj, event) {
    ...
    if (event.dataTransfer) {
        ...
        var tmp_img = $("<img/>").attr("src", "../../" + tmp).load(function(){
		
			//NNE - 20141104
			//the max size is 120*120px
			...
			tmp_img.css("margin", (120 - h) / 2 +"px 0");
			//NNE - 20141104 : END

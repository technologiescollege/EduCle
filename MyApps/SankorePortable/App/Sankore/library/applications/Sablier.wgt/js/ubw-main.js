...
function init(){
    ...
    if(window.sankore){
		...
		tempstep = window.sankore.preference("tempstep","");
		tempstepimg = parseInt(window.sankore.preference("tempstepimg","1"));
		tempsteps = window.sankore.preference("tempsteps", "{}");
		
		tempsteps = JSON.parse(tempsteps);
		
		//alert(tempsteps);
		
		$("#sandglassimg").attr("src","images/sablier/sablier_"+pad(tempstepimg)+".png");
    ...
    .click(function(){
		...
		$("#sandglassimg").attr("src","images/sablier/sablier_01.png");
	...
	function computesteps()
	{
		...
		steplength = timeInSeconds / 10;
		
		tempsteps = [];
		
		for (i=9;i>0;i--)
		{
			tempsteps.push(steplength*i);
		}
		
		window.sankore.setPreference("tempsteps", JSON.stringify(tempsteps));
		
		$("#sandglassimg").attr("src","images/sablier/sablier_01.png");
		//console.log(tempsteps);
		
		tempstep = tempsteps.shift();
		tempstepimg = 1;
	}
	
	function checkstep(time)
	{
		console.log(time+" - "+tempstep+" - "+tempstepimg);
		if (time < tempstep)
		{
			tempstep = tempsteps.shift();
			tempstepimg = tempstepimg+1;
			...
			window.sankore.setPreference("tempstep", tempstep);
			window.sankore.setPreference("tempstepimg", tempstepimg);
			window.sankore.setPreference("tempsteps", JSON.stringify(tempsteps));
    ...
    function updateChronometerReverse(seconds){
        ...
        if(seconds < 1){
            seconds = 0;
			$("#sandglassimg").attr("src","images/sablier/sablier_11.png");
            $("#pausebutton").trigger("click");
        }

package com.lorentz.SVG.display {
	...
	public class SVGDocument extends SVG {
		...
		private static var svgNS : Namespace = new Namespace("http://www.w3.org/2000/svg");
		private static var iaNS : Namespace = new Namespace("http://www.crdp.ac_versailles.fr/2011/images_actives");
		...
		public function parse(svg : XML) : void {
			...
			metadata = SVGParserCommon.parseMetadata(_svg.svgNS::metadata[0]);
		...
		private function visit(elt : XML, target : SVG = null) : SVGElement {
			...
			var localName : String = elt.localName().toLowerCase();

			switch(localName) {
				case 'svg':
					obj = visitSvg(elt, target);
					break;
				case 'defs':
					obj = visitDefs(elt);
					break;
			...
			obj.id = elt.@id;
			obj.title = elt.svgNS::title;
			obj.description = elt.svgNS::desc;
			obj.zoomable = elt.@iaNS::zoomable == "true";
			obj.puce = elt.@iaNS::puce == "true";

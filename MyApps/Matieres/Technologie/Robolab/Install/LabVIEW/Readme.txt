...
NI-VISA 3.0.1 IMPROVEMENTS AND BUG FIXES
----------------------------------------
For a PXI INSTR resource, calling viEnableEvent and viDisableEvent in a loop
would leak memory.  On LabVIEW RT, this would eventually cause a reboot.

For a GPIB INTFC resource, performing any asynchronous I/O would
occasionally cause a crash when you later close the VISA session.

Installing NI-VISA 3.0 on a system with LabVIEW 6.1 would cause user VIs
that use "VISA Configure Serial Port" to have a broken wire.
